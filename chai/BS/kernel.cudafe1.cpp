...
{ 
# 75
__attribute__((unused)) extern XYZ l_mem[]; 
# 76
XYZ *l_in = (l_mem); 
# 77
...
# 85
const int wg_in_J = (((out_size_j - 1) / (blockDim.x)) + (1)); 
# 86
const int wg_in_I = (((out_size_i - 1) / (blockDim.y)) + (1)); 
# 88
for (int i = (((threadIdx.y) * (blockDim.x)) + (threadIdx.x)); i < ((in_size_i + 1) * (in_size_j + 1)); i += ((blockDim.x) * (blockDim.y))) 
# 89
{ 
# 90
(l_in[i]) = (in[i]); 
...
double bj; 
# 102
double mui = (Row / ((double)(out_size_i - 1))); 
# 103
double muj = (Col / ((double)(out_size_j - 1))); 
# 105
